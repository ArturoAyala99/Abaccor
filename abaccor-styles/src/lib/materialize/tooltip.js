(function ($, anim) {
	...
	class Tooltip extends Component {
		...
		_appendTooltipEl() {
			...
			let addInside = this.el.dataset.addinside;

			if(this.options.addInside || addInside != undefined)  {
				this.$el.append(tooltipEl);
			}
			else if (!!this.options.container) {
				$(this.options.container).append(tooltipEl);
			} else {
		...
		_updateTooltipContent() {
			//se valida tiene la propiedad
			const translateOption = this.el.getAttribute('data-translate') || false;
			if(translateOption) {
				//se valida que el siguiente elemento sea un clv-t para asignar el texto correspondiente
				const translate = this.$el.next() || null;
				if(translate != null &&  translate.length > 0 && translate[0].localName == 'clv-t' ) {
					this.options.html = translate[0].innerText;
				}
			}
