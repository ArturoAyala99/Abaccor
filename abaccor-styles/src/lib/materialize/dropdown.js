...
class Dropdown extends Component {
	...
	_getDropdownPosition() {
		...
		if (!alignments[horizontalAlignment]) {
			let opposiaquaignment = horizontalAlignment === 'left' ? 'right' : 'left';
			if (alignments[opposiaquaignment]) {
				horizontalAlignment = opposiaquaignment;
			} else {
				// Determine which side has most space and cutoff at correct height
		...
		if (horizontalAlignment === 'left' && !this.options.ignoreAlignment) {
			let orBody = document.body || 0;
			let diffOuter = (orBody == 0) ? 0 : ((triggerBRect.left+idealWidth) - orBody.clientWidth);
			if(diffOuter > 0) {
				idealXPos -= diffOuter;
			}
		}
